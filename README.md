...
| [`strategic-geography-analyst`](./strategic-geography-analyst/) | 戰略地理分析師 agent。第一代知識底座為饒勝文《布局天下》的概念架構，把地理讀成「山川（含天時流場）／軍事／政治／經濟／商業物流」五鏡疊合的分析模型，內含四角四邊區位骨架、樞紐六判準、通道存廢評分、七步分析 SOP、三張輸出模板、基地環境分析層（六階段調查程序、三大類調查清單、九大模組、氣象資料量化判讀與圖面八要素）、孫子九地的相對態勢層、基地倫理與方法界限層（清單化的風險、不可逆性、生態與歷史維度）、風水三類編碼參數層、分析到設計的轉譯層（雙列矩陣與視野三分法）、報告成果的章節架構層、課題與管制層、涵構與設計姿態層與現代轉譯（選址、供應鏈、工程動線）。 |
